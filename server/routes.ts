...
- All prices are in AED`;
  ...
  // AI Food Pairing
  app.post("/api/restaurants/:restaurantId/ai/pair", async (req, res) => {
    try {
      const restaurantId = parseInt(req.params.restaurantId);
      const { itemName, itemDescription, category } = req.body;

      if (!itemName) {
        return res.status(400).json({ message: "Item name is required for pairing" });
      }

      const menus = await storage.getMenus(restaurantId);
      const allItems = menus.flatMap(m => m.items.filter(i => i.isAvailable !== false));

      // Filter available pairings based on the category of the source item.
      let eligiblePairings = [];
      if (category?.toLowerCase().includes("drink") || category?.toLowerCase().includes("beverage")) {
        // If they view a drink, recommend a main or appetizer.
        eligiblePairings = allItems.filter(i => !i.category?.toLowerCase().includes("drink") && !i.category?.toLowerCase().includes("beverage"));
      } else if (category?.toLowerCase().includes("dessert")) {
        // If they view a dessert, recommend a drink like coffee/tea.
        eligiblePairings = allItems.filter(i => i.category?.toLowerCase().includes("drink") || i.category?.toLowerCase().includes("beverage"));
      } else {
        // For mains/appetizers, recommend drinks or side dishes.
        eligiblePairings = allItems.filter(i => i.category?.toLowerCase().includes("drink") || i.category?.toLowerCase().includes("beverage") || i.category?.toLowerCase().includes("side") || i.category?.toLowerCase().includes("dessert"));
      }

      // Fallback if no specific categories exist
      if (eligiblePairings.length === 0) {
        eligiblePairings = allItems.filter(i => i.name !== itemName);
      }

      const menuContext = eligiblePairings.map(item => {
        return `- ${item.name} (${item.category}) — AED ${item.price}\n  ${item.description || ""}`;
      }).join("\n");

      const prompt = `You are a master sommelier and culinary expert.
The customer is currently looking at this menu item:
Name: ${itemName}
Description: ${itemDescription}
Category: ${category}

Here are the available items on the menu that you can recommend as a pairing:
${menuContext}

Please select the single BEST item from the available list to pair with what the customer is viewing.
Explain briefly in 1-2 sentences WHY they pair so well together (e.g., contrasting flavors, complementary textures, matching regional origins, palate cleansing).
Keep it mouth-watering and enthusiastic!

Return a JSON object with this exact structure:
{
  "recommendedItemName": "Exact name of the item from the menu",
  "reason": "1-2 sentence explanation of why it pairs perfectly"
}`;

      const response = await getOpenAI().chat.completions.create({
        model: "gpt-4o-mini",
        messages: [
          { role: "system", content: "You are an expert food pairing AI. Always return valid JSON." },
          { role: "user", content: prompt }
        ],
        response_format: { type: "json_object" },
      });
      ...
      const pairingData = JSON.parse(content);

      // Find the complete item object details to return to the frontend
      const pairedItem = allItems.find(i => i.name.toLowerCase() === pairingData.recommendedItemName.toLowerCase());

      res.json({
        pairedItem: pairedItem || null,
        reason: pairingData.reason,
        fallbackName: pairingData.recommendedItemName // Just in case name didn't exact match
      });

    } catch (err: any) {
      console.error("AI Pairing Error:", err);
      res.status(500).json({ message: "Failed to generate pairing suggestion." });
    }
  });
